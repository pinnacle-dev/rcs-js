/**
 * This file was auto-generated by Fern from our API Definition.
 */

import * as Pinnacle from "../index.js";

export interface Company {
    /** The unique identifier for the company */
    id?: number;
    /** The date and time when the company was created */
    createdAt?: string;
    /** The name of the company */
    name?: string;
    /** The category of the company */
    category?: Pinnacle.CompanyCategory;
    /** The address of the company */
    address?: string;
    /** The Employer Identification Number (EIN) of the company */
    ein?: string;
    /** A description of the company */
    description?: string;
    /** The brand color of the company */
    brandColor?: string;
    /** The URL of the company's logo */
    logoUrl?: string;
    /** The URL of the company's hero image */
    heroUrl?: string;
    /** The primary website URL of the company */
    primaryWebsiteUrl?: string;
    /** The label for the primary website URL */
    primaryWebsiteLabel?: string;
    /** The primary phone number of the company */
    primaryPhone?: string;
    /** The label for the primary phone number */
    primaryPhoneLabel?: string;
    /** The primary email address of the company */
    primaryEmail?: string;
    /** The label for the primary email address */
    primaryEmailLabel?: string;
    /** The URL of the company's privacy policy */
    privacyPolicyUrl?: string;
    /** The URL of the company's terms of service */
    tosUrl?: string;
    /** The name of the point of contact */
    pocName?: string;
    /** The title of the point of contact */
    pocTitle?: string;
    /** The email address of the point of contact */
    pocEmail?: string;
    /** Explain how users will opt in to receive messages. */
    optIn?: string;
    /** Explain how users will opt out of receiving messages. */
    optOut?: string;
    /** Please provide the unique keywords to opt out. Each keyword should not contain spaces. */
    optOutKeywords?: string[];
    /** Please define what your agent will do. */
    agentUseCase?: string;
    /** Please provide some example messages that your agent will send. */
    expectedAgentResponses?: string;
    /** A list of test phone numbers */
    testNumbers?: string[];
    /** The approval status of the company */
    status?: string;
    /** A list of additional websites */
    additionalWebsites?: Company.AdditionalWebsites.Item[];
    /** A list of additional email addresses */
    additionalEmails?: Company.AdditionalEmails.Item[];
    /** A list of additional phone numbers */
    additionalPhoneNumbers?: Company.AdditionalPhoneNumbers.Item[];
}

export namespace Company {
    export type AdditionalWebsites = AdditionalWebsites.Item[];

    export namespace AdditionalWebsites {
        export interface Item {
            /** The URL of the additional website */
            url?: string;
            /** The label for the additional website */
            label?: string;
        }
    }

    export type AdditionalEmails = AdditionalEmails.Item[];

    export namespace AdditionalEmails {
        export interface Item {
            /** The additional email address */
            email?: string;
            /** The label for the additional email address */
            label?: string;
        }
    }

    export type AdditionalPhoneNumbers = AdditionalPhoneNumbers.Item[];

    export namespace AdditionalPhoneNumbers {
        export interface Item {
            /** The label for the additional phone number */
            label?: string;
            /** The additional phone number */
            phone?: string;
        }
    }
}
